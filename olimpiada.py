...
def build_age_keyboard(prefix: str = "reg") -> InlineKeyboardMarkup:
    """12..30, 30-35, 36-40"""
    ages = [str(i) for i in range(12, 31)] + ["30-35", "36-40"]
    rows, row = [], []
    for a in ages:
        row.append(InlineKeyboardButton(text=a, callback_data=f"{prefix}_age_{a}"))
        if len(row) == 5:
            rows.append(row)
            row = []
    if row:
...
def build_school_keyboard(prefix: str = "reg", page: int = 0, per_page: int = 20) -> InlineKeyboardMarkup:
    """1-maktab .. 102-maktab, sahifalangan"""
    total = 102
    start = page * per_page + 1
    end = min(start + per_page - 1, total)
    rows, row = [], []
    for i in range(start, end + 1):
        row.append(InlineKeyboardButton(text=str(i), callback_data=f"{prefix}_school_{i}-maktab"))
        if len(row) == 5:
            rows.append(row)
            row = []
    if row:
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="⬅️", callback_data=f"{prefix}_schpage_{page-1}"))
...
async def get_required_channels() -> list:
    raw = await get_setting("required_channels", REQUIRED_CHANNEL)
    channels = []
    for part in re.split(r"[,\n;]+", str(raw)):
        ...
        if not ch.startswith("@"):
            ch = "@" + ch.lstrip("@")
        channels.append(ch)
    return channels or [REQUIRED_CHANNEL]
...
async def subscription_keyboard():
    rows = []
    for ch in await get_required_channels():
        rows.append([InlineKeyboardButton(text=f"📢 {ch}", url=f"https://t.me/{ch.replace('@', '')}")])
...
def generate_result_pdf(student_name, test_title, subject, score, pct, correct, wrong, unanswered):
    pdf_buffer = io.BytesIO()
    c = canvas.Canvas(pdf_buffer, pagesize=landscape(letter))
    width, height = 792, 612
    c.setStrokeColor(colors.HexColor("#1A365D"))
    ...
    c.drawCentredString(width / 2, height - 80, "TEST NATIJASI")
    c.setFont("Helvetica-Bold", 18)
    c.setFillColor(colors.HexColor("#2B6CB0"))
    c.drawCentredString(width / 2, height - 130, student_name)
    c.setFillColor(colors.HexColor("#4A5568"))
    c.setFont("Helvetica", 14)
    y = height - 180
    for line in [
        ...
        f"Test: {test_title}",
        f"Ball: {score} ({pct}%)",
        f"Togri: {correct} | Notogri: {wrong} | Javobsiz: {unanswered}",
        f"Sana: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M')} UTC",
    ]:
        c.drawCentredString(width / 2, y, line)
        y -= 28
    c.setFont("Helvetica-Oblique", 11)
    c.drawCentredString(width / 2, 50, "Professional Olimpiada")
    c.save()
    pdf_buffer.seek(0)
    return pdf_buffer.read()
...
class AdminEditQuestionState(StatesGroup):
...
async def get_main_menu_keyboard():
    ...
    keyboard.extend([
        ...
        [KeyboardButton(text="❌ Xato javoblar"), KeyboardButton(text="🏫 Maktab reytingi")],
...
def get_admin_menu():
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="🏆 Admin reyting"), KeyboardButton(text="🏫 Maktab reytingi")],
            [KeyboardButton(text="🔍 O'quvchini qidirish")],
...
async def cmd_start(message: Message, state: FSMContext, bot: Bot):
    ...
    if not is_subscribed:
        ...
        await message.answer(f"⚠️ Botdan foydalanish uchun avval quyidagi kanal(lar)ga obuna bo'lishingiz kerak:\n\n{ch_list}", reply_markup=keyboard)
        return
...
async def process_self_fullname(message: Message, state: FSMContext, bot: Bot):
    ...
    await message.answer("👤 Yoshingizni tanlang:", reply_markup=build_age_keyboard("reg"))
...
async def reg_school_page(callback: CallbackQuery, state: FSMContext):
    if await state.get_state() != SelfRegState.waiting_for_school.state:
        await callback.answer()
        return
    page = int(callback.data.replace("reg_schpage_", "", 1))
    try:
        await callback.message.edit_reply_markup(reply_markup=build_school_keyboard("reg", page))
    except Exception:
        await callback.message.answer("🏫 Maktabingizni tanlang:", reply_markup=build_school_keyboard("reg", page))
    await callback.answer()


@router.callback_query(F.data.startswith("reg_school_"))
async def reg_pick_school(callback: CallbackQuery, state: FSMContext):
    if await state.get_state() != SelfRegState.waiting_for_school.state:
        await callback.answer()
        return
    school = callback.data.replace("reg_school_", "", 1)
    ...
    try:
        await callback.message.edit_text(
            ...
            f"Yosh: {data['age']} | Sinf: {data['grade']} | Maktab: {school}"
        )
    except Exception:
        pass
...
async def profile_edit_fullname(message: Message, state: FSMContext, bot: Bot):
    ...
    await message.answer("👤 Yoshingizni tanlang:", reply_markup=build_age_keyboard("pedit"))


@router.callback_query(F.data.startswith("pedit_age_"))
async def pedit_pick_age(callback: CallbackQuery, state: FSMContext):
    if await state.get_state() != ProfileEditState.waiting_for_age.state:
        await callback.answer()
        return
    age = callback.data.replace("pedit_age_", "", 1)
    await state.update_data(age=age)
    await state.set_state(ProfileEditState.waiting_for_grade)
    try:
        await callback.message.edit_text(f"✅ Yosh: <b>{age}</b>\n\n📚 Sinfingizni tanlang:", reply_markup=build_grade_keyboard("pedit"))
    except Exception:
        await callback.message.answer(f"✅ Yosh: <b>{age}</b>\n\n📚 Sinfingizni tanlang:", reply_markup=build_grade_keyboard("pedit"))
    await callback.answer()


@router.callback_query(F.data.startswith("pedit_grade_"))
async def pedit_pick_grade(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        await callback.message.edit_text(
            f"✅ Sinf: <b>{grade}</b>\n\n🏫 Maktabingizni tanlang (1–102):",
            reply_markup=build_school_keyboard("pedit", 0)
        )
    except Exception:
        await callback.message.answer(
            f"✅ Sinf: <b>{grade}</b>\n\n🏫 Maktabingizni tanlang (1–102):",
...
@router.callback_query(F.data.startswith("pedit_schpage_"))
async def pedit_school_page(callback: CallbackQuery, state: FSMContext):
    if await state.get_state() != ProfileEditState.waiting_for_school.state:
        await callback.answer()
        return
    page = int(callback.data.replace("pedit_schpage_", "", 1))
    try:
        await callback.message.edit_reply_markup(reply_markup=build_school_keyboard("pedit", page))
    except Exception:
        await callback.message.answer("🏫 Maktabingizni tanlang:", reply_markup=build_school_keyboard("pedit", page))
    await callback.answer()


@router.callback_query(F.data.startswith("pedit_school_"))
async def pedit_pick_school(callback: CallbackQuery, state: FSMContext):
    if await state.get_state() != ProfileEditState.waiting_for_school.state:
        await callback.answer()
        return
    school = callback.data.replace("pedit_school_", "", 1)
    data = await state.get_data()
    fullname_parts = data["fullname"].split(" ", 1)
    first_name = fullname_parts[0]
    last_name = fullname_parts[1] if len(fullname_parts) > 1 else "-"
...
async def pedit_text_fallback(message: Message, state: FSMContext, bot: Bot):
    if message.text in ("🏠 Asosiy menyu", "⬅️ Bosh menyu", "🚀 Start"):
        await cmd_start(message, state, bot)
        return
    st = await state.get_state()
    if st == ProfileEditState.waiting_for_age.state:
        await message.answer("👆 Yuqoridagi ro'yxatdan yoshni tanlang:", reply_markup=build_age_keyboard("pedit"))
    elif st == ProfileEditState.waiting_for_grade.state:
        await message.answer("👆 Yuqoridagi ro'yxatdan sinfni tanlang:", reply_markup=build_grade_keyboard("pedit"))
    else:
        await message.answer("👆 Yuqoridagi ro'yxatdan maktabni tanlang:", reply_markup=build_school_keyboard("pedit", 0))
...
async def show_test_analysis_for_appeal(callback: CallbackQuery):
    ...
    async with async_session() as session:
        ...
        keyboard.append([InlineKeyboardButton(text="🎓 Sertifikat", callback_data=f"get_cert_{ts.id}"), InlineKeyboardButton(text="📄 Natija PDF", callback_data=f"get_result_pdf_{ts.id}")])
...
async def finish_block_test_by_student(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    try:
        await callback.message.answer(
            "Natijangizni PDF yuklab olishingiz mumkin:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📄 Natija PDF", callback_data=f"get_result_pdf_{ts.id}")]
            ])
        )
    except Exception:
        pass
        ...
        async with async_session() as final_session:
            ...
            if sess and sess.status == "IN_PROGRESS":
                ...
                try:
                    await bot.send_message(
                        chat_id=user_id,
                        text="Natijangizni PDF yuklab olishingiz mumkin:",
                        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                            [InlineKeyboardButton(text="📄 Natija PDF", callback_data=f"get_result_pdf_{sess.id}")]
                        ])
                    )
                except Exception:
                    pass
...
async def admin_student_details(callback: CallbackQuery):
    ...
    async with async_session() as session:
        ...
        if not sessions:
            ...
            kb = [[InlineKeyboardButton(text="🚫 Bloklash" if student.is_active else "✅ Blokdan chiqarish",
                  callback_data=f"adm_block_{student.id}" if student.is_active else f"adm_unblock_{student.id}")]]
        ...
        if student.is_active:
            ...
            keyboard.append([InlineKeyboardButton(text="✅ Blokdan chiqarish", callback_data=f"adm_unblock_{student.id}")])
        await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard))
        await callback.answer()
...
async def admin_save_answers_and_test(message: Message, state: FSMContext, bot: Bot):
    ...
    async with async_session() as session:
        ...
        await session.commit()
        saved_test_id = new_test.id
        saved_title = new_test.title
        saved_subject = new_test.subject
        saved_grades = new_test.grade_level
        saved_is_block = is_block
    await state.clear()
    await message.answer(
        f"✅ Test muvaffaqiyatli saqlandi!\n📚 {saved_subject} — {saved_title}\nSinflar: <b>{saved_grades}</b>",
        reply_markup=get_admin_menu()
    )
    # Mos sinf / Abituriyent o'quvchilariga bildirishnoma
    try:
        grades_set = set(parse_grades(saved_grades))
        async with async_session() as session:
            all_st = (await session.execute(
                select(Student).where(
            ...
            if saved_is_block:
                ...
                targets = [s for s in all_st if s.grade in grades_set]
        bot = message.bot
        n = 0
        for st in targets:
            try:
                await bot.send_message(
                    st.telegram_id,
                    f"📢 <b>Yangi test ochildi!</b>\n\n"
                    f"📚 {saved_subject} — <b>{saved_title}</b>\n"
                    f"Sinf: {saved_grades}\n\n"
                    f"Menyudan «📝 Testni boshlash» orqali kirishingiz mumkin."
                )
                n += 1
                await asyncio.sleep(0.04)
            except Exception:
                pass
        await message.answer(f"📣 <b>{n}</b> ta o'quvchiga xabar yuborildi.")
    except Exception as e:
    ...
    async with async_session() as session:
        ...
        for t in tests:
            ...
            keyboard.append([
                ...
                InlineKeyboardButton(text="🏁 Yakun", callback_data=f"finish_test_{t.id}"),
                InlineKeyboardButton(text="🗑", callback_data=f"delete_test_{t.id}")
            ])
            keyboard.append([
                InlineKeyboardButton(text="📋 Nusxa", callback_data=f"clone_test_{t.id}"),
                InlineKeyboardButton(text="✏️ Savollar", callback_data=f"edit_qs_{t.id}")
            ])
...
async def live_statistics(message: Message):
    ...
    async with async_session() as session:
        ...
        in_progress = await session.scalar(select(func.count(TestSession.id)).where(TestSession.status == "IN_PROGRESS"))
        blocked = await session.scalar(select(func.count(Student.id)).where(Student.is_active == False))
        pending_pay = await session.scalar(select(func.count(Payment.id)).where(Payment.status == "PENDING"))
        await message.answer(
            f"📊 <b>Jonli statistika:</b>\n\n"
            f"👥 Jami o'quvchilar: {total}\n"
            f"🚫 Bloklangan: {blocked or 0}\n"
            f"✅ Test topshirganlar: {completed or 0}\n"
            f"⏳ Hozir ishlayotgan: {in_progress or 0}\n"
            f"💳 Kutilayotgan to'lov: {pending_pay or 0}"
        )
...
async def download_result_pdf(callback: CallbackQuery, bot: Bot):
    ...
    async with async_session() as session:
        ...
        pdf_bytes = generate_result_pdf(
            f"{student.first_name} {student.last_name}", test.title, test.subject,
            ts.score, ts.score_percentage, ts.correct_answers, ts.wrong_answers, ts.unanswered
        )
        doc = BufferedInputFile(pdf_bytes, filename=f"natija_{student.student_id}.pdf")
        await bot.send_document(callback.message.chat.id, document=doc, caption="📄 Sizning test natijangiz")
        await callback.answer()


@router.message(F.text == "🏫 Maktab reytingi")
async def school_rating_menu(message: Message, state: FSMContext):
    if await state.get_state() == TestProcessState.in_test.state:
        return
    async with async_session() as session:
        tests = (await session.execute(select(Test))).scalars().all()
        if not tests:
            await message.answer("Hozircha testlar yo'q.")
            return
        kb = [[InlineKeyboardButton(text=f"🏫 [{t.grade_level}] {t.subject}", callback_data=f"school_rate_{t.id}")] for t in tests[:20]]
        await message.answer("🏫 <b>Maktablar reytingi</b>\n\nTestni tanlang:", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))


@router.callback_query(F.data.startswith("school_rate_"))
async def show_school_rating(callback: CallbackQuery):
    test_id = int(callback.data.split("_")[2])
    async with async_session() as session:
        test = await session.get(Test, test_id)
        rows = (await session.execute(
            select(Student.school, func.avg(TestSession.score), func.count(TestSession.id))
            .join(TestSession, Student.id == TestSession.student_id)
            .where(TestSession.test_id == test_id, TestSession.status == "COMPLETED", Student.school.is_not(None))
            .group_by(Student.school)
            .order_by(func.avg(TestSession.score).desc())
            .limit(15)
        )).all()
        if not rows:
            await callback.message.edit_text("Bu test bo'yicha maktab natijalari yo'q.")
            await callback.answer()
            return
        text = f"🏫 <b>Maktab reytingi: {test.subject}</b>\n\n"
        for idx, (school, avg_score, cnt) in enumerate(rows, 1):
            medal = "🥇" if idx == 1 else ("🥈" if idx == 2 else ("🥉" if idx == 3 else f"{idx}."))
            text += f"{medal} <b>{school or '-'}</b> — o'rtacha <b>{round(float(avg_score or 0), 2)}</b> ({cnt} o'quvchi)\n"
        await callback.message.edit_text(text)
        await callback.answer()
...
async def admin_channels_prompt(message: Message, state: FSMContext):
    if not await is_admin(message.from_user.id):
        return
    current = await get_setting("required_channels", REQUIRED_CHANNEL)
    await state.set_state(AdminChannelsState.waiting_for_channels)
    await message.answer(
        f"📢 <b>Majburiy kanallar</b>\n\nHozirgi: <code>{current}</code>\n\n"
        f"Yangi ro'yxat: <code>@kanal1, @kanal2</code>\n(Bekor: 🏠 Asosiy menyu)",
        reply_markup=get_cancel_to_menu_keyboard()
    )


@router.message(AdminChannelsState.waiting_for_channels)
async def admin_save_channels(message: Message, state: FSMContext, bot: Bot):
    if message.text in ("🏠 Asosiy menyu", "⬅️ Bosh menyu", "🚀 Start"):
        await cmd_start(message, state, bot)
        return
    if not await is_admin(message.from_user.id):
        return
    await set_setting("required_channels", message.text.strip())
    await state.clear()
    channels = await get_required_channels()
    await message.answer("✅ Kanallar:\n" + "\n".join(f"• {c}" for c in channels), reply_markup=get_admin_menu())


@router.callback_query(F.data.startswith("clone_test_"))
async def clone_test(callback: CallbackQuery, bot: Bot):
    if not await is_admin(callback.from_user.id):
        return
    test_id = int(callback.data.split("_")[2])
    async with async_session() as session:
        src = await session.get(Test, test_id)
        if not src:
            await callback.answer("Topilmadi!", show_alert=True)
            return
        questions = list((await session.execute(select(Question).where(Question.test_id == src.id).order_by(Question.id))).scalars().all())
        clone = Test(
            title=src.title + " (nusxa)", subject=src.subject, grade_level=src.grade_level,
            max_attempts=src.max_attempts, mode=src.mode, duration_minutes=src.duration_minutes,
            question_time_seconds=src.question_time_seconds, is_block_test=src.is_block_test,
            block_subjects=src.block_subjects, start_time=None, end_time=None, is_active=True, is_finished=False,
        )
        session.add(clone)
        await session.flush()
        for q in questions:
            session.add(Question(
                test_id=clone.id, section_name=q.section_name, question_text=q.question_text,
                photo_file_id=q.photo_file_id, option_a=q.option_a, option_b=q.option_b,
                option_c=q.option_c, option_d=q.option_d, correct_option=q.correct_option, points=q.points,
            ))
        await session.commit()
        grades_set = set(parse_grades(clone.grade_level))
        all_st = (await session.execute(select(Student).where(Student.telegram_id.is_not(None), Student.is_active == True))).scalars().all()
        targets = all_st if clone.is_block_test else [s for s in all_st if s.grade in grades_set]
        n = 0
        for st in targets[:500]:
            try:
                await bot.send_message(st.telegram_id, f"📢 <b>Yangi test!</b>\n\n📚 {clone.subject} — {clone.title}\nSinf: {clone.grade_level}")
                n += 1
                await asyncio.sleep(0.03)
            except Exception:
                pass
    await callback.answer(f"✅ Nusxa yaratildi! {n} xabar.", show_alert=True)


@router.callback_query(F.data.startswith("edit_qs_"))
async def admin_edit_questions_start(callback: CallbackQuery, state: FSMContext):
    if not await is_admin(callback.from_user.id):
        return
    test_id = int(callback.data.split("_")[2])
    async with async_session() as session:
        questions = list((await session.execute(select(Question).where(Question.test_id == test_id).order_by(Question.id))).scalars().all())
        if not questions:
            await callback.answer("Savollar yo'q!", show_alert=True)
            return
        kb = []
        for i, q in enumerate(questions[:30], 1):
            prev = (q.question_text[:40] + "…") if len(q.question_text) > 40 else q.question_text
            kb.append([InlineKeyboardButton(text=f"{i}. {prev}", callback_data=f"edit_one_q_{q.id}")])
        await callback.message.answer(f"✏️ Savolni tanlang ({len(questions)} ta):", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
        await callback.answer()


@router.callback_query(F.data.startswith("edit_one_q_"))
async def admin_edit_one_question(callback: CallbackQuery, state: FSMContext):
    if not await is_admin(callback.from_user.id):
        return
    qid = int(callback.data.split("_")[3])
    async with async_session() as session:
        ...
        await state.set_state(AdminEditQuestionState.waiting_for_new_text)
        await callback.message.answer(
            f"✏️ Hozirgi:\n<b>{q.question_text}</b>\nA) {q.option_a}\nB) {q.option_b}\nC) {q.option_c}\nD) {q.option_d}\nTo'g'ri: {q.correct_option}\n\n"
            f"Yangi matn:\n<code>Savol\nA) ...\nB) ...\nC) ...\nD) ...\nTo'g'ri: A</code>",
            reply_markup=get_cancel_to_menu_keyboard()
        )
        await callback.answer()
...
@router.message(AdminEditQuestionState.waiting_for_new_text)
async def admin_save_edited_question(message: Message, state: FSMContext, bot: Bot):
    if message.text in ("🏠 Asosiy menyu", "⬅️ Bosh menyu", "🚀 Start"):
        await cmd_start(message, state, bot)
        return
    if not await is_admin(message.from_user.id):
    ...
    parsed = parse_single_question_text(message.text)
    correct = "A"
    m = re.search(r"to['']?g['']?ri\s*[:\-]\s*([A-D])", message.text, re.I)
    if m:
    ...
    async with async_session() as session:
        ...
        if not q:
            ...
            return
        if parsed:
            q.question_text, q.option_a, q.option_b = parsed["text"], parsed["a"], parsed["b"]
            q.option_c, q.option_d = parsed.get("c") or "C", parsed.get("d") or "D"
        else:
            q.question_text = message.text.strip()
        q.correct_option = correct
        await session.commit()
    await state.clear()
    await message.answer("✅ Savol yangilandi!", reply_markup=get_admin_menu())
...
async def reminder_scheduler(bot: Bot):
    while True:
        ...
        try:
            ...
            # Blok test: ~30 daqiqada vaqt eslatmasi
            async with async_session() as session:
                active_blocks = (await session.execute(
                    select(TestSession, Test, Student)
                    .join(Test, TestSession.test_id == Test.id)
                    .join(Student, TestSession.student_id == Student.id)
                    .where(TestSession.status == "IN_PROGRESS", Test.is_block_test == True, Student.telegram_id.is_not(None))
                )).all()
                for ts, t, st in active_blocks:
                    started = ts.started_at
                    if started.tzinfo is None:
                        started = started.replace(tzinfo=timezone.utc)
                    ...
                    if elapsed_min in (30, 60, 90, 120, 150) and (elapsed % 60) < 70:
                        try:
                            await bot.send_message(
                                st.telegram_id,
                                f"⏳ <b>Vaqt eslatmasi</b>\n\n🧩 {t.title}\nQolgan vaqt: <b>{int(remaining // 60)} daqiqa</b>"
                            )
                        except Exception:
                            pass
        except Exception as e:
